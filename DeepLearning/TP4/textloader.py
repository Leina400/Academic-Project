...
def pad_collate_fn(samples: List[List[int]]):
  ...
  #  print("phrases avant eof et padding :\n", samples)

    for i in range(len(samples)):
        # on ajoute a la fin EOS
        samples[i] = torch.cat([samples[i], torch.tensor([EOS_IX])])

    # phrase la plus longue dans le sample
    max_len = max(seq.size(0) for seq in samples)

    padded = []
    for seq in samples:
        longueur = seq.size(0)

        while longueur < max_len:
            seq = torch.cat([seq, torch.tensor([PAD_IX], dtype=seq.dtype)])
            longueur += 1

        padded.append(seq)


    batch = torch.stack(padded).T

   # print("phrases apres eof et padding :\n", batch)
    return batch
